...
export function useAuth(): AuthHookResult {
  ...
  const fetchUserProfile = useCallback(async (userId: string): Promise<UserProfile | null> => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL || 'http://localhost:54321'}/functions/v1/auth-profile`, {
        method: 'GET',
        headers: {
  ...
  const signUp = useCallback(async (data: RegisterData): Promise<void> => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL || 'http://localhost:54321'}/functions/v1/auth-register`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const resetPassword = useCallback(async (email: string): Promise<void> => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL || 'http://localhost:54321'}/functions/v1/auth-reset-password`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },

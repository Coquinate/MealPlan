...
export function SoundToggle() {
  ...
  return (
    <button
      onClick={toggleSound}
      className="fixed bottom-4 right-4 bg-surface-glass backdrop-blur-md border border-surface-glass-border rounded-full p-3 shadow-lg hover:bg-surface-glass-elevated transition-all duration-200 z-50"
      title={soundEnabled ? "Dezactivează sunetele" : "Activează sunetele"}
    >
      {soundEnabled ? (
        <IconVolume2 size={20} className="text-primary-warm" />
      ) : (
        <IconVolumeOff size={20} className="text-text-muted" />
      )}
    </button>
  )
}
